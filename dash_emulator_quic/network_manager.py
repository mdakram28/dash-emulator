...
from exp_common.exp_events import ExpEvent_BwSwitch
...
class NetworkConfig:
    ...
    def apply(self, if_name):
        ...
        tc qdisc change dev {if_name} handle 3: netem limit {NETEM_LIMIT} delay {self.latency}ms 0ms loss {float(self.drop) * 0.5:.3f}%
        '''
        t = round(time() * 1000)
...
class NetworkManager:
    ...
    def get_server_container(self):
        ...
        if not self.current_container["Names"].endswith("_client_1"):
            raise Exception("Failed to get current container")
        docker_compose_proj = self.current_container["Names"][:-len("_client_1")]
        server_container_name = docker_compose_proj + "_server_1"
